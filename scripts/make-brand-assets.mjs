// One-shot generator for CargoPro brand assets.
// Source: logo-cargopro-transparent.png (repo root, 707x353, transparent).
...
const SRC = join(ROOT, 'logo-cargopro-transparent.png');
...
await writeFile(join(PUB, 'logo-cargopro-horizontal-transparent.png'), horizPng);
await sharp(horizPng).avif({ quality: 60, effort: 7 }).toFile(join(PUB, 'logo-cargopro-horizontal-transparent.avif'));
await sharp(horizPng).webp({ quality: 82, effort: 6 }).toFile(join(PUB, 'logo-cargopro-horizontal-transparent.webp'));
...
await writeFile(join(PUB, 'logo-cargopro-square.png'), await squareIcon(512, TRANSPARENT, 0.1));
...
console.log('✓ CargoPro brand assets generated in public/');
